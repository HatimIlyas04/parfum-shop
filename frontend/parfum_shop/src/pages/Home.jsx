  ...
  style.textContent = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .as-hero-eyebrow {
      ...
      margin-bottom: 20px;
      animation: as-up 0.9s ease both;
    }
    .as-hero-title {
      font-family: 'Playfair Display', serif;
    ...
    .as-sec-line {
      width: 56px; height: 2px; background: #D4AF37;
    ...
    .as-filter-bar {
      ...
      margin-bottom: 52px; border-bottom: 1px solid #ebebeb;
      flex-wrap: wrap;
    }
    .as-filter-btn {
    ...
    /* Image container — FIXED HEIGHT, uniform */
    .as-prod-img-wrap {
      position: relative;
      width: 100%;
      height: 260px;
    ...
    .as-card-cart-btn:hover svg { transform: scale(1.1); color: #fff; }
    ...
    .as-footer-desc { font-size: 11.5px; line-height: 1.8; max-width: 240px; }
    .as-footer-col-title {
    ...
    .as-footer-col ul li a {
      color: rgba(255,255,255,0.42); text-decoration: none; font-size: 11.5px;
    ...
    /* ── RESPONSIVE ── */
    /* Desktop 4 cols ≥ 1200px */
    @media (min-width: 1200px) {
      .as-product-grid { grid-template-columns: repeat(4, 1fr); }
    }
    /* Tablet 3 cols 900–1199 */
    @media (min-width: 900px) and (max-width: 1199px) {
      .as-product-grid { grid-template-columns: repeat(3, 1fr); gap: 22px; }
    }
    /* Tablet small 541–899 → 2 cols */
    ...
    /* Mobile → strictly 2 cols */
    @media (max-width: 768px) {
      .as-navbar { padding: 0 18px; height: 62px; }
      .as-mobile-menu { top: 62px; }
      .as-nav-links, .as-nav-actions { display: none !important; }
      .as-hamburger { display: flex !important; }
      .as-hero { margin-top: 62px; min-height: 480px; height: 78vh; }
...
export default function Home() {
  ...
  const filteredProducts = (() => {
    switch (filter) {
      case "homme":  return products.filter(p => p.category === "homme");
      case "femme":  return products.filter(p => p.category === "femme");
      case "mixte":  return products.filter(p => p.category === "mixte");
...
<nav className={`as-navbar ${scrolled ? "scrolled" : ""}`}>
  ...
  <div className="as-nav-actions">
    <button className="as-cart-btn" onClick={() => navigate("/cart")} aria-label="Panier">
      <CartIcon size={18} />
    ...
    <span /><span /><span />
      ...
      <div className={`as-mobile-menu ${menuOpen ? "open" : ""}`}>
        ...
        <div className="as-mobile-actions">
          <button className="as-cart-btn" onClick={() => { navigate("/cart"); setMenuOpen(false); }} aria-label="Panier">
            <CartIcon size={16} />
            {cartCount > 0 && <span className="as-cart-badge">{cartCount}</span>}
          </button>
        ...
        {loading ? (
          ...
          <div className="as-product-grid">
            {filteredProducts.length === 0 ? (
              <p className="as-empty">Aucun parfum dans cette catégorie.</p>
            ) : (
